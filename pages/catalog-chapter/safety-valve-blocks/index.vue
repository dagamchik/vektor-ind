    ...
    <section class="catalog">
      <div class="container container__catalog">
        <div class="catalog__block">
          <div class="link__way">
            <nuxt-link to="/" class="main__link">Главная страница</nuxt-link> - <nuxt-link to="/catalog" class="catalog__link">Каталог</nuxt-link> - <p>Блоки предохранительных клапанов</p>
          </div>
          <h2>Блоки предохранительных клапанов</h2>
        </div>

        <div class="main__block">
          <filterBlockSafetyValve :products="products" @updateProducts="doUpdate"/>
...
import filterBlockSafetyValve from "@/components/filterBlockSafetyValve.vue";
...
export default {
  ...
  components: {
    ...
    footerComponent,
    productChapter,
    filterBlockSafetyValve
  },
  methods: {
    fetchData() {
      this.$axios.$get(`/api/safety-valve-blocks`)
        .then((res) => {
          this.products = res;
    ...
    doUpdate(newArr) {
      ...
      // this.$emit('updatePaginate', this.products);
    },
    sortItem() {
      let vm = this;
      if(vm.sortType === 'name') {
